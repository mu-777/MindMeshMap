...
| エッジ選択 | エッジをクリック（ノードの選択とは排他。選択したエッジは`Delete`キーで削除可能） |
...
エッジの削除方法は3通りあります: (1) エッジをクリックして選択し `Delete` キーを押す、(2) 選択したエッジのラベル横に表示される `×` ボタンをクリックする、(3) エッジを右クリック（モバイルは長押し）してメニューから削除する。
