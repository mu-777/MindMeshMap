...
有向グラフ構造（循環許容）のマインドマップをブラウザ上で編集できるWYSIWYGエディタです。

## 概要

従来のマインドマップはツリー構造（木構造）に限定されていますが、本エディタは**有向グラフ（DAG: Directed Acyclic Graph）** を拡張し、**循環を含むグラフ構造**もサポートします。これにより、概念間の相互関係や循環的な依存関係を自然に表現できます。
...
保存されたマップはGoogle Drive内の `MindMapDAG` フォルダに格納されます。
