...
ノードを編集中にテキストを選択すると、書式バー（BubbleMenu）が表示されます。
...
ツールバーの「ファイル」メニュー（モバイルでは `⋮` メニュー内）から以下の操作ができます。

| 操作 | 内容 |
|------|------|
| JSONエクスポート | 現在のマップをJSONファイルとしてダウンロード |
| PNGエクスポート | 現在のマップをPNG画像としてダウンロード |
| JSONインポート | JSONファイルを選択してマップを読み込み（インポート後はDrive未保存の新規マップとして扱われます） |
