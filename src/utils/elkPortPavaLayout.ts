// 整列アルゴリズム「elk-port-pava」（方針G': ポート制約付き階層レイアウトの最小構成実装）。
// フェーズごとの入出力を含む詳細仕様はdocs/align-algorithms.md §7、
...
//
// **`elk-port-ext`（方針G）との違い**: `elk-port-ext` は「ELKと同じ結果を出す」ことを目標に
// ELK layered のパイプラインを忠実に再実装したもの。こちらは同じ枠組みを**ELKに寄せずに**
// 素直に書いた版で、座標割当をBrandes–Köpfではなく等調回帰(PAVA)で解き、原点へ正規化せず
// 入力の外接矩形の左上に留まる。結果としてELKとは違う（多くの指標でより良い）配置になる。
// 両者を並べて比較できるようにするため、方式として残してある
// （[decisions.md §48](../../docs/decisions.md)）。
//
...
/**
 * 「elk-port-pava」アルゴリズムのエントリポイント。
 * ポート制約付き階層レイアウトを、ELKに依存せず自前で計算する（同期処理）
 */
export function calculateElkPortPavaLayout(
  nodes: MapNode[],
  edges: MapEdge[],
  direction: LayoutDirection
): LayoutResult {
