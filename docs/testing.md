...
| `edge-drop-handle-side.mjs` | ハンドルから空白へドラッグして作った新規ノードの受け口の面と位置（decisions.md §52） | RIGHTの4面＋DOWNの3面から引き伸ばして、エッジの`sourceHandle`が掴んだハンドルのまま／新規ノードの`targetHandle`が「backward起点なら forward 面、それ以外（forward・cross）なら backward 面」になること、**ドロップ点が新規ノードのどの面になるか**（backward起点なら forward 面＝RIGHTで右端・DOWNで下端、それ以外は左上）が許容4px以内で合うこと。位置のアサートは`EMPTY_NODE_WIDTH`/`EMPTY_NODE_HEIGHT`（`src/utils/nodeContent.ts`）とCustomNodeの実寸のズレも検出する（CSSを変えて定数を更新し忘れたら落ちる）。ハンドル割り当てはDOMから読めないためlocalStorageのドラフト（`mindmeshmap-draft`）を読む。ドロップ先が空白であること・ノードが1個増えたことを併せてアサートし、ドロップ先はノードから90px以上離れた点を走査して選ぶ（方向を切り替えると配置が変わるため固定座標では狙えない）。ケースごとに独立したページを使う（1ページで続けて引くと新規ノードが初期マップと重なり狙ったハンドルを掴めなくなる） | 自動 |
